 ...
 */
export class MongoDB extends Transport {
  constructor(options) {
    super(options);
    options = (options || {});
    if (!options.db) {
    ...
    function processOpQueue() {
      self._opQueue.forEach(({method, args}) => self[method].apply(self, args));
    ...
    function createCollection(db) {
      ...
      return db.createCollection(self.collection, opts).then(col=>{
        ...
        return col.indexInformation({full: true}).then(info=>{
          info = info.filter(({name}) => name === ttlIndexName);
  ...
  log(info, cb) {
    ...
    process.nextTick(()=>{
      ...
      const {message: messageOriginal, level, ...meta} = info; 
   ...
   * @return {Stream}
   */
  streamPoll(options = {}, stream = new Stream) {
    let self = this;
    let start = options.start;
    let last;
    if (!this.logDb) {
